...
def draw_rectangle(draw, x1, y1, x2, y2, outline_color, width, fill_color):
    draw.rectangle([x1, y1, x2, y2], outline=outline_color, width=width, fill=fill_color)

def draw_circle_with_text(draw, x, y, text, color, parking_lot_width, parking_lot_height):
    circle_radius = min(parking_lot_width, parking_lot_height) // 2
    circle_center = (x + circle_radius, y + circle_radius)

    # Draw the rectangle behind the circle
    rectangle_coords = [
        x,
        y,
        x + parking_lot_width,
        y + parking_lot_height
    ]
...
def generate_parking_lot_image(data):
    ...
    if 'slots' in data:
        ...
        for i, slot in enumerate(slots):
            ...
            draw_circle_with_text(draw, x1 + 1, y1 + 20, lot_name, color, parking_lot_width, parking_lot_height)
